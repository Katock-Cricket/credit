...
export function createProfileView(root) {
  ...
  $("pf-init").addEventListener("click", () => {
    ...
    act(call("init", { homeUrl }), t("msgInitDone"));
  });
  $("pf-sync").addEventListener("click", () => act(call("sync", {}), t("msgSyncDone")));
  $("pf-confirm").addEventListener("click", () => act(call("confirm", {}), t("msgConfirmDone")));
  $("pf-discard").addEventListener("click", () => act(call("discardDraft", {}), t("msgDiscardDone")));
  $("pf-update-pr").addEventListener("click", () => {
    const prId = prompt(t("promptPrId"));
    if (!prId) return;
    act(call(`updateFromPr?prId=${encodeURIComponent(prId)}`, {}), t("msgUpdateDone"));
  });
