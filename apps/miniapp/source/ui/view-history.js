 ...
 *
 * 布局：左侧历史 PR 列表 / 右侧泳道甘特图 + 分析图层 + Task 详情。
 * 数据全部来自 `credit.listPrs` / `credit.getTaskStageView` / `credit.getBehaviors`
 * （P4：一律经 transport，D-044 —— 不再直连 HTTP）。
 */
...
export function createHistoryView(root, transport) {
  const $ = (id) => root.querySelector(`#${id}`);
  /** P4：一律经 transport（D-044） */
