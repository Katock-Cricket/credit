/**
 * 协作模式画像（P2-pre T7，决策 D-017）。
 *
 * 按 Task 序列的结构特征把一次 PR 归到四类原型之一：
 *
 * | 模式 | 特征 |
 * |---|---|
 * | 巡航式 cruise | prompt 少、AI 工具调用密集、人主要做验收 |
 * | 结对式 pair | prompt 频繁、人机交替密集 |
 * | 审阅式 review | AI 产出密集 + 人的阅读行为密集 |
 * | 手工式 manual | Dev 自己的编辑占主导 |
 *
 * P1 样例数据（283 agent.tool / 47 edit|ai / 10 prompt / 1 edit|dev）落在**巡航式**。
 */
...
export const PATTERN_LABELS: Record<CollabPattern, { "zh-CN": string; "en-US": string }> = {
...
export const PATTERN_DESC: Record<CollabPattern, { "zh-CN": string; "en-US": string }> = {
  cruise: {
...
export function createCollabPatternLayer(): AnalyticLayer {
  return {
    id: "collab-pattern",
    ...
    compute(graph: TaskGraph, _behaviors: Behavior[]): AnalyticView {
      ...
      for (const t of graph.tasks) {
        ...
        for (const [action, n] of Object.entries(t.counts)) {
          if (action === "agent.tool") toolCalls += n;
          if (action === "view" || action === "file.scroll" || action === "cursor") reads += n;
        }
      ...
      const signals = {
        ...
        devEditRatio:
          allEditLines > 0 ? Number((devEditLines / allEditLines).toFixed(4)) : 0,
        readRatio: total > 0 ? Number((reads / total).toFixed(4)) : 0,
        promptPerTask:
          graph.tasks.length > 0 ? Number((prompts / graph.tasks.length).toFixed(2)) : 0,
        toolCalls,
      };

      let pattern: CollabPattern = "unknown";
      if (total > 0) {
        if (signals.devEditRatio > THRESHOLD.devEditDominant) pattern = "manual";
        else if (signals.readRatio >= THRESHOLD.readFocused && signals.aiRatio > 0.5)
          pattern = "review";
        else if (signals.promptPerTask >= THRESHOLD.promptPerTask) pattern = "pair";
        else pattern = "cruise";
      }
