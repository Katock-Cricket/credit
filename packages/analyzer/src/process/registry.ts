...
function unavailableView(layer: AnalyticLayer, message: string): AnalyticView {
  return {
    id: layer.id,
    summary: "该分析不可用",
    data: null,
...
export function createAnalyticRegistry(layers: AnalyticLayer[]): AnalyticRegistry {
  const byId = new Map(layers.map((l) => [l.id, l]));

  return {
    layers,
    runAll(graph, behaviors) {
      const out: AnalyticView[] = [];
      for (const l of layers) {
        try {
          out.push(l.compute(graph, behaviors));
        } catch (e) {
          out.push(unavailableView(l, String((e as Error)?.message ?? e)));
        }
      }
      return out;
    },
    run(id, graph, behaviors) {
      ...
      try {
        ...
        return unavailableView(l, String((e as Error)?.message ?? e));
      }
    },
  };
}
